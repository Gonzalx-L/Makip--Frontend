...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({ 
  ...
  const handleImageSubmit = async (e: React.FormEvent) => {
    ...
    const safetyTimeout = setTimeout(() => {
      console.error('⏰ Timeout de seguridad activado - deteniendo loading');
      setLoading(false);
      setFormError('La operación tardó demasiado tiempo. Por favor, intenta nuevamente.');
    }, 30000); // 30 segundos
      ...
      if (selectedImage) {
        try {
          ...
          if (uploadResult.isApproved === true) {
            ...
            // Si es pickup, mostrar pantalla de confirmación con código
            if (deliveryMethod === 'pickup') {
              console.log('🏪 Procesando recojo en tienda con código:', response.pickup_code);
              setOrderResult(response);
              setStep('store-pickup-confirmation');
              clearTimeout(safetyTimeout); // Limpiar timeout
              setLoading(false); // Importante: detener el loading
              // Para pickup, NO llamamos onPaymentSuccess aquí - se llamará desde la pantalla de confirmación
            } else {
              // Solo para delivery llamamos onPaymentSuccess directamente
              clearTimeout(safetyTimeout); // Limpiar timeout
              onPaymentSuccess(response.order_id.toString());
            }
          } else {
            ...
            console.log('⚠️ Configurando error en el formulario:', friendlyMessage);
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-gray-900 bg-white px-3 py-1 rounded border font-bold text-lg">
                      {orderResult?.pickup_code || 'No disponible'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-green-700">
                      S/ {orderResult?.total_price ? Number(orderResult.total_price).toFixed(2) : formatPrice(total)}
                    </span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="space-y-3">
                ...
                <div className="flex space-x-3">
                  <button
                    onClick={() => {
                      // Para recojo, limpiar carrito y ir directo al inicio sin pasar por PaymentSuccessModal
                      clearCart();
                      onClose();
                      navigate('/');
                    }}
                    className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-3 px-6 rounded-lg transition-colors flex items-center justify-center space-x-2"
                  >
                  ...
                  <button
                    onClick={() => window.open('https://wa.me/51981266608?text=Hola%2C%20tengo%20un%20pedido%20para%20recojo%20en%20tienda', '_blank')}
